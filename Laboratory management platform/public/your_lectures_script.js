...
//populate table with infos
	...
	for(var j=0;j<labs_codes.length;j++){
		await loadStudents(labs_codes[j]);
		var nn = await getLabNameById(labs_codes[j]);
		var lab_title = document.createElement("TD");
		lab_title.innerHTML = "LABORATORY "+nn.toUpperCase();
		lab_title.style.textAlign="center";
		lab_title.style.backgroundColor="#E08119";
	    lab_title.style.borderBottom="1px solid black";
	    lab_title.colSpan="33";
		 
		table.appendChild(lab_title);
...
//append to current row attendeces and points of student for each week
	  ...
	  //asign a and p 
	  attendences.innerHTML = a.toString();
	  var mBox = document.createElement("TD");
	  if(currentStudents[i].midterm != undefined)
	  	mBox.innerHTML = currentStudents[i].midterm;
	  var mExam = document.createElement("TD");
	  if(currentStudents[i].finalGrade!=undefined)
	  	mExam.innerHTML = currentStudents[i].finalGrade;
	  if(currentStudents[i].extraPoints!=undefined)
	  	p = p + parseInt(currentStudents[i].extraPoints,10);
	  points.innerHTML = p.toString();
	  row2.appendChild(attendences);
	  row2.appendChild(points);
	  row2.appendChild(mBox);
	  row2.appendChild(mExam);
	  table.appendChild(row2);
	}
	document.getElementById("download").style.display="";
}
...
if (sessionStorage.getItem("is_reloaded")){ 
	document.getElementById("myInput").value = getCookie("lecture");
	await load_labs();
	if(!arra.includes(getCookie("lecture")))return;
	selectedLectureIndex = parseInt(getCookie("lecture_id"),10);
	await loadLabsCodes();
	await getLabInfos();
}
}
