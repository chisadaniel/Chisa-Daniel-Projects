...
def compute_delta_x(ai, value):
    ...
    return (n*compute_p_of_x(ai, value))/(first_der+np.sign(first_der)*math.sqrt(abs(compute_h_of_x(ai, value))))

def generate_xi_values(n):
    vector = [1.0, -6.0, 11.0, -6.0]
    #for i in range(n+1):
...
def laguere_method(ai, x):
    ...
    while eps <= abs(compute_delta_x(ai, x)) <= 10 ** 8 and k <= kmax:
        h_x = compute_h_of_x(ai, x)
        if h_x < 0:
            break
        first_der = compute_first_derivative(ai, x)
        if abs(first_der+np.sign(first_der)*math.sqrt(abs(h_x))) <= eps:
            break
...
def compute_all_solutions(ai):
    ...
    for i in range(100):
        ...
        if solution is not False and round(solution, 0) not in solutions:
            solutions.append(round(solution, 0))
